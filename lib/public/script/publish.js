...
Aurita.Publish = { 
  ...
  init_marginal_placement_editor : function(page_id, sections) { 
    ...
    try { Sortable.destroy('banner_placements'); } catch(e) { } 
    ...
    Sortable.create('place_banner_selection_list', { 
      dropOnEmpty: true, 
      handle: 'header', 
      containment: [ 'place_banner_selection_list', 'banner_placements' ] 
    });
    Sortable.create('banner_placements', { 
      dropOnEmpty: true, 
      handle: 'header', 
      onUpdate: function(container) { 
        placements = Sortable.serialize(container.id); 
        Aurita.call({ method: 'POST', 
                      action: 'Advert::Banner_Placement/perform_add/page_id='+page_id+'&'+placements });
      }, 
      containment: [ 'place_banner_selection_list', 'banner_placements' ] 
    });
